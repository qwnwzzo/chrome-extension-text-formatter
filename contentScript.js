chrome.runtime.onMessage.addListener((request) => {
  if (request && request.ops == 'format') {
    modifySelectedInputText("123456");
  }
});

const modifySelectedInputText = (replacementText) => {
  const activeElement = document.activeElement;
  const tagName = activeElement.tagName.toLowerCase();
  
  // 确保活动元素是 input 或 textarea
  if (activeElement && (tagName === 'textarea' || (tagName === 'input' && activeElement.type === 'text'))) {
    ...
    if (startPos !== null && endPos !== null && startPos !== endPos) {
      ...
      const afterText = originalText.substring(endPos, originalText.length);
      const selectedText = originalText.substring(startPos, endPos);
      const replacementText = textFormatter.spacing(selectedText);
      
      // 设置新的值
      activeElement.value = beforeText + replacementText + afterText;
      
      // 更新光标位置
      activeElement.selectionStart = activeElement.selectionEnd = startPos + replacementText.length;
    }
  }
}
